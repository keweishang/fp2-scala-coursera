...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filterNot {
      case (block, history) => explored contains block
    }
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        ...
        (neighborBlock, neighborHistory) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield (neighborBlock, neighborHistory)
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (block, history) #:: _ => history.reverse
  }
}
